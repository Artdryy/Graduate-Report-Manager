...
class CompaniesController {
  createCompany = async (req, reply) => {
    ...
    const result = await CompaniesService.createCompany({ company_name, description, address, phone_number, email });
  ...
  updateCompany = async (req, reply) => {
    ...
    const result = await CompaniesService.updateCompany({ company_id, company_name, description, address, phone_number, email });
  ...
  deleteCompany = async (req, reply) => {
    ...
    reply.code(200).sendSuccess({ message: result.message || 'Company deleted', data: null });
  };
}
